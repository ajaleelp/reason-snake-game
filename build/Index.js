...
/******/ 	__webpack_require__.p = "/Users/jlee/work/reason-snake-game/build/";
/******/
...
var Direction$SnakeGame = __webpack_require__(40);
var Caml_builtin_exceptions = __webpack_require__(0);

function newHead(head, direction) {
  var newX = Cell$SnakeGame.x(head) + Direction$SnakeGame.deltaX(direction) | 0;
  var newY = Cell$SnakeGame.y(head) + Direction$SnakeGame.deltaY(direction) | 0;
  return Cell$SnakeGame.create(/* tuple */[
              newX,
              newY
            ]);
...
function foodFound(head, food) {
  var foodPosition = Food$SnakeGame.position(food);
  if (Cell$SnakeGame.x(head) === Cell$SnakeGame.x(foodPosition)) {
    return +(Cell$SnakeGame.y(head) === Cell$SnakeGame.y(foodPosition));
  } else {
    return /* false */0;
  }
}

function fixTail(hasEaten, tail) {
  if (hasEaten !== 0) {
    return tail;
  } else {
    return List.tl(tail);
...
function newTail(snake, hasEaten) {
  var match = snake[/* body */0];
  if (match) {
    return List.append(/* :: */[
                match[0],
                /* [] */0
              ], List.rev(fixTail(hasEaten, List.rev(match[1]))));
  } else {
    throw [
...
function move(snake, direction, food) {
  var match = snake[/* body */0];
  if (match) {
    var head$prime = newHead(match[0], direction);
    var hasEaten = foodFound(head$prime, food);
    return /* record */[
            /* body : :: */[
              head$prime,
              newTail(snake, hasEaten)
            ],
            /* direction */direction
          ];
  } else {
...
function create(xs, direction) {
  return /* record */[
          /* body */List.map(Cell$SnakeGame.create, xs),
          /* direction */direction
        ];
}
...
function direction(snake) {
  return snake[/* direction */1];
}

function head(snake) {
  return List.hd(snake[/* body */0]);
}

function grew(oldSnake, newSnake) {
  return +(List.length(newSnake[/* body */0]) > List.length(oldSnake[/* body */0]));
}
...
var initialWorld = World$SnakeGame.create(initialSnake, initialFood);
...
function handleTick() {
  ...
  var oldSnake = World$SnakeGame.snake(oldWorld);
  var newSnake = Snake$SnakeGame.move(oldSnake, Snake$SnakeGame.direction(oldSnake), World$SnakeGame.food(oldWorld));
  var match = Snake$SnakeGame.grew(oldSnake, newSnake);
  var newFood = match !== 0 ? Food$SnakeGame.createRandom(/* () */0) : World$SnakeGame.food(oldWorld);
  var newWorld = World$SnakeGame.create(newSnake, newFood);
  drawWorld(newWorld);
  console.log(Snake$SnakeGame.direction(newSnake));
  return /* () */0;
}
...
function handleEvent(evt) {
  ...
  var newDirection = Direction$SnakeGame.update(Snake$SnakeGame.direction(oldSnake), keyPressed);
  var newSnake = Snake$SnakeGame.move(World$SnakeGame.snake(oldWorld), newDirection, World$SnakeGame.food(oldWorld));
  var newWorld = World$SnakeGame.create(newSnake, World$SnakeGame.food(oldWorld));
  state[0] = newWorld;
  return /* () */0;
}
...
/* initialSnake Not a pure module */
...
function update(direction, key) {
  var match = parsedKey(key);
  var exit = 0;
  if (match) {
    if (direction !== 1) {
      if (direction >= 3) {
        ...
        var match$1 = match[0];
        if (match$1 !== 1 && match$1 < 3) {
          return direction;
        } else {
          return Ext$SnakeGame.Option[/* unsafelyUnwrapOption */1](match);
        }
      }
    } else {
      exit = 1;
...
var Curry = __webpack_require__(1);
var Js_exn = __webpack_require__(44);
var Caml_array = __webpack_require__(19);
var Caml_exceptions = __webpack_require__(31);
var Caml_builtin_exceptions = __webpack_require__(0);
...
var Char = __webpack_require__(41);
